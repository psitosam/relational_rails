...
RSpec.describe 'the podcasts index page' do
  it "visitor sees each podcast in the system including each podcast's attributes" do
    ...
    podcast_5 = harris.podcasts.create!(title: "Making Sense", topic: "Recipes For Future Plagues", length_in_minutes: 118, favorites: false)
    ...
    podcast_7 = weingart.podcasts.create!(title: "EMCrit Podcast", topic: "Ep. 319: Safe and Smart Reversal of Anticoagulation Agents in 2022", length_in_minutes: 9, favorites: false)

    podcast_8 = weingart.podcasts.create!(title: "On Deeper Reflection", topic: "Email Tactics", length_in_minutes: 37, favorites: false)
    ...
    expect(page).to have_content(podcast_1.podcaster.name)
    ...
    expect(page).to have_content(podcast_2.podcaster.name)
    ...
    expect(page).to have_content(podcast_3.podcaster.name)
    ...
    expect(page).to have_content(podcast_4.podcaster.name)
    ...
    expect(page).to have_content(podcast_5.podcaster.name)
    ...
    expect(page).to have_content(podcast_6.podcaster.name)
    ...
    expect(page).to have_content(podcast_7.podcaster.name)
    ...
    expect(page).to have_content(podcast_8.podcaster.name)
    ...
    expect(page).to have_content(podcast_9.podcaster.name)
